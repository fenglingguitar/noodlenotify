...
public class ExchangeTraceCacheConnectManager extends ConnectManagerAbstract {

	private final static Logger logger = LoggerFactory.getLogger(ExchangeTraceCacheConnectManager.class);
	
	public ExchangeTraceCacheConnectManager(ConnectAgentFactory connectAgentFactory) {
	...
	protected void updateConnectAgent() {
		...
		try {
			consoleInfoMap = consoleRemotingInvoke.exchangerGetTraceStorages(moduleId);
		} catch (Exception e) {
			if (logger.isErrorEnabled()) {
				logger.error("UpdateConnectAgent -> "
							+ "ModuleId: " + moduleId
							+ ", Exchanger Get TraceStorages -> " + e);
			}
		}
			...
			if (logger.isDebugEnabled()) {
				...
				for (String queueName : set) {
					...
					for (QueueTraceStorageVo queueTraceStorageVo : list) {
						logger.debug("UpdateConnectAgent -> ExchangerGetTraceStorages -> " 
									+ "ModuleId: " + moduleId
									+ ", QueueName: " + queueName 
