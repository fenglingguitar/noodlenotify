...
public class DistributeBodyCacheConnectManager extends ConnectManagerAbstract {

	private final static Logger logger = LoggerFactory.getLogger(DistributeBodyCacheConnectManager.class);
	
	public DistributeBodyCacheConnectManager(ConnectAgentFactory connectAgentFactory) {
	...
	protected void updateConnectAgent() {
		...
		try {
			consoleInfoMap = consoleRemotingInvoke.distributerGetMsgBodyCaches(moduleId);
		} catch (Exception e) {
			...
			if (logger.isDebugEnabled()) {
				...
				for (String queueName : set) {
					...
					for (QueueMsgBodyCacheVo queueMsgBodyCacheVo : list) {
						logger.debug("UpdateConnectAgent -> DistributerGetMsgBodyCaches -> List -> " 
									+ "ModuleId: " + moduleId
									+ ", QueueName: " + queueName 
