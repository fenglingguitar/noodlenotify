  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/trace/tracestorage/querypage',
				datatype: 'json',
				mtype: 'post',
			   	colNames: [
			   	        '编号',
			   	...
			   	colModel: [
					{name:'traceStorage_Id', index:'traceStorage_Id', width:100, align: 'center'},
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					var vo = new Object();
					vo['traceStorage_Id'] = ret['traceStorage_Id'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/trace/queue/tracestorage/queryqueue',
						datatype: "json",
						mtype: 'post',
		...
		function insert() {
			top.openDialog('跟踪数据库新增', '<%=request.getContextPath()%>/view/trace/tracestorage/tracestorage_edit.jsp', null, 260, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('跟踪数据库修改', '<%=request.getContextPath()%>/view/trace/tracestorage/tracestorage_edit.jsp', ret, 260, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/trace/tracestorage/deletes',
				jsonSet: jsonSet
			});	
		}
			...
			<div id="form" style="width: auto;">
				<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
				    ...
				    <tr>
				    	...
				    	<td><input type="text" id="traceStorage_Id" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')"/></td>
				    	<th><label>跟踪数据库名称</label></th>
