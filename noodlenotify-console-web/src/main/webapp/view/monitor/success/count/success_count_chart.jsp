  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function callback(trnId, data, other) {
			...
			if (trnId == "CHART_QUERY") {
				...
				for (i=0; i<data.length; i++) {
					totalSeriesData.push([data[i].timestamp, data[i].totalCount]);
					errorSeriesData.push([data[i].timestamp, data[i].totalCount - data[i].successCount]);
				}
		...
		function treeHighlightEvent(data, label, node) {
			...
			if (data.other == 'EXCHANGE' || data.other == 'DISTRIBUTE') {
				...
				var divId = ppppid + '_' + 
							...
							pid + '_' + 
							id;
				var title = node.parent.parent.parent.label + '-' + 
		...
		function buildChart(obj) {
			...
			transaction({
				id: 'GET_DATETIME',
				url: '<%=request.getContextPath()%>/monitor/chart/success/getdatetime',
				async: false,
				other: obj.renderTo
			});
	                    ...
	                    load: function() {
                        	...
                        	if (intervalStart == true) {
                        		var intervalProcess = setInterval(function() {
    	                        	...
    	                        	$.ajax({
    	                        		url: '<%=request.getContextPath()%>/monitor/chart/success/querychartsinglenowlast',
    	                        		...
    	                        		success: function(data, textStatus, jqXHR) {
    	                        			...
    	                        			for (i=0; i<data.length; i++) {
    	                                        ...
    	                                        var y = data[i].totalCount;
    	                                        options.series[0].addPoint([x, y], true, true);
    	                                        
    	                                        var x = data[i].timestamp;
    	                                        var y = data[i].totalCount - data[i].successCount;
		...
		function queryChart(obj) {
			...
			if ($('#region').val() != '') {
				...
				intervalStart = true;
				url = '<%=request.getContextPath()%>/monitor/chart/success/querychartsinglenow';
			} else {
				if ($('#catch_Tm_Start').val() != ''
						&& $('#catch_Tm_End').val() != '') {
					var catch_Tm_Start = 
					...
					intervalStart = false;
					url = '<%=request.getContextPath()%>/monitor/chart/success/querychartbganded';
				} else {
					regionCurrent = 60;
					jsonSet.put('region', regionCurrent);
					intervalStart = true;
		...
		function fullscreen() {
			...
			openChildWindowPost(
					'<%=request.getContextPath()%>/view/monitor/success/count/success_count_chart_fullscreen.jsp', 
					'urlParam',
