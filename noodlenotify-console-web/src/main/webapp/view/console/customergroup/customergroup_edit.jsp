    ...
    <script type="text/javascript">
		...
		function init() {
			...
			if (urlParamObject != null) {				
				...
				$('#customerGroup_Nm').attr('disabled', 'true');
				initModel = 'UPDATE';
			} else {
				initModel = 'INSERT';
			}
		...
		function save() {
			...
			if (initModel == 'INSERT') {	
				transaction({
					id: 'INSERT',
					url: '<%=request.getContextPath()%>/console/customergroup/insert',
					jsonSet: jsonSet
				});
			} else {
				transaction({
					id: 'UPDATE',
					url: '<%=request.getContextPath()%>/console/customergroup/update',
					jsonSet: jsonSet
				});				
			}
		}
	...
	<div>
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout_input">
			    ...
			    <tr>
			    	<th><label id="customerGroup_Nm_Label">消费组名称</label></th>
