  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/producer/querypage',
				datatype: 'json',
				mtype: 'post',
			   	colNames: [
			   	        '编号',
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/producer/deletes',
				jsonSet: jsonSet
			});	
		}
		...
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
			    ...
			    <tr>
			    	...
			    	<td><input type="text" id="producer_Id" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')"/></td>
			    	<th><label>生产者名称</label></th>
