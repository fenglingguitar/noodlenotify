  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/consumer/querypage',
				datatype: 'json',
				mtype: 'post',
			   	colNames: [
			   	        '编号',
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					var vo = new Object();
					vo['consumer_Id'] = ret['consumer_Id'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/queue/consumer/queryqueue',
						datatype: "json",
						mtype: 'post',
		...
		function insert() {
			top.openDialog('消费中心新增', '<%=request.getContextPath()%>/view/console/consumer/consumer_edit.jsp', null, 335, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('消费中心修改', '<%=request.getContextPath()%>/view/console/consumer/consumer_edit.jsp', ret, 335, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/consumer/deletes',
				jsonSet: jsonSet
			});	
		}
		...
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
			    ...
			    <tr>
			    	...
			    	<td><input type="text" id="consumer_Id" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')"/></td>
