  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/msgbodycache/querypage',
				datatype: 'json',
				mtype: 'post',
			   	colNames: [
			   	        '编号',
			   	...
			   	colModel: [
					{name:'msgBodyCache_Id', index:'msgbodycache_Id', width:100, align: 'center'},
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					var vo = new Object();
					vo['msgBodyCache_Id'] = ret['msgBodyCache_Id'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/queue/msgbodycache/queryqueue',
						datatype: "json",
						mtype: 'post',
		...
		function insert() {
			top.openDialog('消息体缓存新增', '<%=request.getContextPath()%>/view/console/msgbodycache/msgbodycache_edit.jsp', null, 260, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('消息体缓存修改', '<%=request.getContextPath()%>/view/console/msgbodycache/msgbodycache_edit.jsp', ret, 260, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/msgbodycache/deletes',
				jsonSet: jsonSet
			});	
		}
		...
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
			    ...
			    <tr>
			    	...
			    	<td><input type="text" id="msgBodyCache_Id" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')"/></td>
			    	<th><label>消息体缓存名称</label></th>
