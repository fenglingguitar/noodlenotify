  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			var paramObject = {'consumerGroup_Nm': urlParamObject.consumerGroup_Nm};
			...
			$('#list_left').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/consumergroup/queryincludepage',
				datatype: 'json',
				mtype: 'post',
			...
			$('#list_right').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/consumergroup/queryexcludepage',
				datatype: 'json',
				mtype: 'post',
				postData:{'input': jsonToString(paramObject)}, 
			   	colNames: [
		...
		function inserts() {
			...
			for (var i=0; i<indexArray.length; i++) {				
				...
				ret['consumerGroup_Nm'] = $('#consumerGroup_Nm').val();
				retArray.push(ret);
			}
			...
			transaction({
				id: 'INSERTS',
				url: '<%=request.getContextPath()%>/console/consumer/updates',
				jsonSet: jsonSet
			});	
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/consumer/deletesgroup',
				jsonSet: jsonSet
			});
		}
					...
					<div id="expander_left" class="s_expander">
						...
						<div id="form_left" style="width: auto;">
							<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
							    ...
							    <tr>
							    	<th><label>消费者组</label></th>
							    	<td><input type="text" id="consumerGroup_Nm" maxlength="32" disabled="true"/></td>
