  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/consumergroup/querypage',
				datatype: 'json',
			   	colNames: [
						'消费组名称', 
						'控制状态'
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/queue/consumergroup/queryqueue',
						datatype: "json",
						mtype: 'post',
		...
		function insert() {
			top.openDialog('消费组新增', '<%=request.getContextPath()%>/view/console/consumergroup/consumergroup_edit.jsp', null, 225, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('消费组修改', '<%=request.getContextPath()%>/view/console/consumergroup/consumergroup_edit.jsp', ret, 225, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/consumergroup/deletes',
				jsonSet: jsonSet
			});	
		}
		...
		function setConsumer() {
			...
			var ret = jQuery('#list').jqGrid('getRowData', index);
			top.openDialog('设置消费者', '<%=request.getContextPath()%>/view/console/consumergroup/consumergroup-consumer/consumergroup_consumer_main.jsp', ret, 670, 1200, null);					
		}
