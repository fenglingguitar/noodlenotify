  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/exchanger/querypage',
				datatype: 'json',
				mtype: 'post',
			   	colNames: [
			   	        '编号',
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					var vo = new Object();
					vo['exchanger_Id'] = ret['exchanger_Id'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/queue/exchanger/queryqueue',
						datatype: "json",
						mtype: 'post',
		...
		function insert() {
			top.openDialog('交换中心新增', '<%=request.getContextPath()%>/view/console/exchanger/exchanger_edit.jsp', null, 305, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('交换中心修改', '<%=request.getContextPath()%>/view/console/exchanger/exchanger_edit.jsp', ret, 305, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/exchanger/deletes',
				jsonSet: jsonSet
			});	
		}
		...
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
			    ...
			    <tr>
			    	...
			    	<td><input type="text" id="exchanger_Id" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')"/></td>
			    	<th><label>交换中心名称</label></th>
