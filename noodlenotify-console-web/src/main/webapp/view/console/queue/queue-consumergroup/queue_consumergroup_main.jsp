  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list_left').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/queue/consumergroup/queryincludepage',
				datatype: 'json',
				mtype: 'post',
				postData:{'input': jsonToString(paramObject)}, 
			   	...
			   	colModel: [
					...
					{name:'consumerGroup_Nm', index:'consumerGroup_Nm', width:300, align: 'center'},
					{name:'consumer_Num', index:'consumer_Num', width:300, align: 'center'},
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					vo['consumerGroup_Nm'] = ret['consumerGroup_Nm'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/queue/consumergroup/queryconsumerforqueue',
						datatype: "json",
						mtype: 'post',
						...
						colNames: [
						   	...
						   	colModel: [
								{name:'consumer_Id', index:'consumer_Id', width:100, align: 'center'},
			...
			$('#list_right').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/queue/consumergroup/queryexcludepage',
				datatype: 'json',
				mtype: 'post',
				postData:{'input': jsonToString(paramObject)}, 
			   	colNames: [
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
					...
					vo['consumerGroup_Nm'] = ret['consumerGroup_Nm'];
					jQuery("#"+subgrid_table_id).jqGrid({
						url:'<%=request.getContextPath()%>/console/consumer/querypagebyequal',
						datatype: "json",
						mtype: 'post',
						postData:{'input': jsonToString(vo)}, 
						colNames: [
		...
		function queryLeft() {
			...
			if($("#consumerGroup_Nm_left").val() !== ''){
				var reg = /^(\d|[a-z|A-Z])+$/;
				var r = $("#consumerGroup_Nm_left").val().match(reg); 
				if(r === null){
					alert('左侧请输入正确的消费者组名称(字母或数字)');
					return;
				}
			}
			if($("#consumer_Num_left").val() !== ''){
				var reg = /^\d+$/;
				var r = $("#consumer_Num_left").val().match(reg); 
				if(r === null){	
		...
		function inserts() {
			...
			transaction({
				id: 'INSERTS',
				url: '<%=request.getContextPath()%>/console/queue/consumergroup/inserts',
				jsonSet: jsonSet
			});	
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/queue/consumergroup/deletes',
				jsonSet: jsonSet
			});	
		}
						...
						<div id="form_left" style="width: auto;">
							<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
							    ...
							    <tr>
							    	...
							    	<td><input type="text" id="consumerGroup_Nm_left" maxlength="32"/></td>
								</tr>
								<tr>
							    	...
							    	<td><input type="text" id="consumer_Num_left" maxlength="32"/></td>
							    </tr>
							</table>
						</div>
					</div>
