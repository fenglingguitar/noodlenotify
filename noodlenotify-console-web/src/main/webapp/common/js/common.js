var Sys = new Object();
Sys.ie 		= navigator.userAgent.indexOf("MSIE") > 0 || navigator.userAgent.indexOf("Trident") > 0 ? true : false;
...
function expandAdjustGrid(defaultHeight, form, expander, list, queryDiv) {
	var expanderChangeSize = $("#" + form + " table tr").size() * 31 + 6;
	repaintGrid(defaultHeight, expander, list, queryDiv, expanderChangeSize );
}

function collapseAdjustGrid(defaultHeight, expander, list, queryDiv) {
	repaintGrid(defaultHeight, expander, list, queryDiv);
}
...
function repaintGrid(defaultHeight, expander, list, queryDiv, expanderChangeSize ) {
	...
	$("#" + list + "_div .ui-jqgrid-bdiv").height(maxHeight);
}
